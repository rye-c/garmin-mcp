...
| Lactate threshold HR | Two distinct numbers, not one — see note below. **LT2 (functional/anaerobic threshold, ~1hr race effort): 167bpm** (single Garmin algorithmic reading, unverified by a real test). **LT1 (aerobic threshold, empirically derived): ~150bpm** — cross-checked 2026-09-03 against Garmin's independent training-effect labels across 116 runs: AEROBIC_BASE tops out at 148bpm (n=63), every run labeled LACTATE_THRESHOLD sat at 150-161bpm (n=11, no exceptions below 150). Use **LT1 (~150bpm)** to judge whether a run is genuinely easy/Zone 2 — the current 149bpm Zone 2 ceiling sits right at this line, which validates it rather than requiring a change. Use **LT2 (167bpm)** only as the target intensity when the plan calls for actual tempo/threshold work later (Build phase), not for judging easy days. | 2026-08-26 (LT2) / 2026-09-03 (LT1, empirical) |
